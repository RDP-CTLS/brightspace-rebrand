// Step 3: one main download chosen by what Merge did, the fallback second, a one-line check result,
...
const run=async(mode,tick)=>{await setFile(new File([b],'course-export.zip'));await w(100);
  if(tick)$q('#merge-all').click();
...
const R={};
R.bothNone=await run('both',false);
R.bothAll=await run('both',true);
R.restyle=await run('restyle',false);
