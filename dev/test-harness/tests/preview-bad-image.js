...
// Needs ORIGINAL = a course whose content page carries a deflated (method 8) image (a full
// real course export). PASS = original.paneUpdated && styled.paneUpdated && errs [].
...
for(const t of $a('#expl-list .ctopic').filter(x=>!x.classList.contains('cintro'))){
  ...
  if(target)break;}
if(!victim)return{skipped:'no deflated content-page image in this fixture — needs a full real course export'};
